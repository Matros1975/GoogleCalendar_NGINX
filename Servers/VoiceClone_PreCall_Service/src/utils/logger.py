"""
Logging configuration for VoiceClone Pre-Call Service.

Reuses the pattern from ElevenLabsWebhook service with enhancements for voice cloning context.
"""
...
# Context variables to store call/clone context across async calls
call_context: ContextVar[str] = ContextVar('call_id', default='N/A')
caller_context: ContextVar[str] = ContextVar('caller_id', default='N/A')


class VoiceCloneFilter(logging.Filter):
    """Add call_id and caller_id to all log records from context."""
    
    def filter(self, record: logging.LogRecord) -> bool:
        """Inject call_id and caller_id into log record."""
        record.call_id = call_context.get()
        record.caller_id = caller_context.get()
        return True
...
class StandardFormatter(logging.Formatter):
    """Standard text formatter for human-readable logs with context."""
    
    def __init__(self):
        """Initialize formatter."""
        super().__init__(
            fmt="%(asctime)s - [Call:%(call_id)s|Caller:%(caller_id)s] - %(name)s - %(levelname)s - %(message)s",
            datefmt="%Y-%m-%d %H:%M:%S"
        )
...
def setup_logger(
    ...
    log_dir = os.getenv("LOG_DIR", "/var/log/mcp-services")
    ...
    # Create context filter (will be added to each handler)
    context_filter = VoiceCloneFilter()
...
def set_call_context(call_id: str, caller_id: str = "N/A") -> None:
    """
    Set call and caller context for logging.
    ...
    caller_context.set(caller_id)


def clear_call_context() -> None:
    """Clear call and caller context."""
    call_context.set("N/A")
    caller_context.set("N/A")
